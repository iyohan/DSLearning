...
assisstance in the copletion of this assignment.
Signed: John Lee
*/
...
pNode half(pList p) {
	...
	/*
	int N = size(p);
	for(pNode c = begin(p), int i = 0; i < N / 2 + 1; c = c->next, i++);
	return c;
	*/
	pNode rabbit = begin(p);
	pNode turtle = begin(p);
	//while(rabbit != end(p) && rabbit != nullptr){
...
void push(pList p, int val, int x) {
	...
	pNode next_node = find(p, x);
	insert(next_node, val);
...
void reverse(pList p) {
	...
	pNode node = nullptr;
	for(node = p->head; node != p->tail; node = node->prev)
...
void shuffle(pList p) {
	...
	pNode que_next = nullptr;
	pNode node_next = nullptr;
	for(pNode node = begin(p); que != nullptr; que = que_next, node = node_next){
		que_next = que->next;
		node_next = node->next;
		que->prev = node;
		que->next = node->next;
...
void push_sorted(pList p, int val) {
	...
	if(!sorted(p)) return;
	pNode c = begin(p);
	if(c->item <= c->next->item)  //if sorted in ascending order
		c = _more(p, val);
	else
		c = _less(p, val);
	insert(c, val);
