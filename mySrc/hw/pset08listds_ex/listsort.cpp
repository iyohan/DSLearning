...
void selectionSort(pList p, int(*comp)(int, int)) {
	...
	for(pNode wall = begin(p); wall->next != end(p); wall = wall->next){
		pNode wall_c = wall;
		for(pNode moving = wall->next; moving != end(p); moving = moving->next){
			if(comp(moving->item, wall_c->item) < 0)
				wall_c = moving;
		}
		std::swap(wall->item, wall_c->item);
	}
