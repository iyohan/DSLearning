...
int _binary_search(int *data, int key, int lo, int hi) {
	...
	int mid_idx = (hi + lo) / 2;
	if(hi < lo) return -lo;
	if(key > data[mid_idx]) return _binary_search(data, key, mid_idx + 1, hi);
	if(key < data[mid_idx]) return _binary_search(data, key, lo, mid_idx - 1);
	return mid_idx;
}
...
int main(int argc, char *argv[]) {
	...
	for(i = 0; i < size; i++){
		key = rand() % size;
		answer = binary_search(list, key, size);
		if(answer < 0)
			cout << "\t" << key << "\tis not@[" << -answer << "]\n";
		else
			cout << "\t" << key << "\tis @[" << answer << "]\n";
	}
	return 0;
